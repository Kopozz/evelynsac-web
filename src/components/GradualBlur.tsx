...
const useResponsiveDimension = (responsive: boolean, config: Record<string, unknown>, key: string): string => {
...
function GradualBlur(props: GradualBlurProps) {
  ...
  const responsiveHeight = useResponsiveDimension(config.responsive || false, config as unknown as Record<string, unknown>, 'height');
  const responsiveWidth = useResponsiveDimension(config.responsive || false, config as unknown as Record<string, unknown>, 'width');
  ...
  const containerStyle = useMemo((): React.CSSProperties => {
    ...
    if (isVertical) {
      ...
      baseStyle[(config.position || 'bottom') as 'top' | 'bottom'] = 0;
      baseStyle.left = 0;
      baseStyle.right = 0;
    } else if (isHorizontal) {
      baseStyle.width = responsiveWidth || responsiveHeight;
      baseStyle.height = '100%';
      baseStyle[(config.position || 'bottom') as 'left' | 'right'] = 0;
      baseStyle.top = 0;
       baseStyle.bottom = 0;
    }
